...
class Peer(object):
    ...
    def get_self_utxo(self):
        return [utxo for utxo in self.utxo_set.values()
                if (utxo.vout.to_addr in self.wallet.addrs)]
    ...
    def create_transaction(self,peer,
        ...
        if tx_type == 'normal':
            to_addr = self.receiver_addr_choice(peer)
            ...
            if outputs:
                ...
                logger.info('{0}(pid={1}) created a transaction to peer{2} for {3} Bitcoin'.format(self,self.pid,peer.pid, value))
                # logger.info('txid:{0}'.format(self.current_tx.id))

                return True
            return False
        
    def receiver_addr_choice(self,peer):
        tmp = []
        idx = -1
        for i in range(peer.wallet.nok):
            if peer.wallet.valid[i] == 0:
                tmp.append(i)
        if not tmp:
...
def create_normal_tx(peer,to_addr,value) :
    ...
    # no enough money for transaction
    if balance  < value:
...
def broadcast_tx(peers,current_tx):    
    ...
    if rand < Params.SLOW_PEERS_IN_NETWORK:
        choice = [random.choice(idxs)] # one integer
    if rand < Params.SLOWER_PEERS_IN_NETWORK:
        choice = random.sample(idxs,k = 2) # two integer

    for i,peer in enumerate(peers): 
        if peer._delayed_tx: # if exist delayed_tx, randomly add into peer.mem_pool
            # dice = random.uniform(0,1)
            # if dice > Params.SLOW_PEERS_IN_NETWORK:
            #     fill_mem_pool(peer)
            fill_mem_pool(peer)

        if peer.verify_transaction(current_tx,peer.mem_pool): # if tx not in peer.mem_pool
...
def find_vin_pointer_from_tx(tx):
    return [vin.to_spend for vin in tx.tx_in]
