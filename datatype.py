...
class UTXO(tuple):
    ...
    def to_json(self):
        keys_u = ['to_addr', 'Pointer', 'is_coinbase']
        keys_p = ['tx_id', 'n']
        keys_t = ['addr',"value"]
        values_p = [self[1][0],self[1][1]]
        values_t = [self[0][0],self[0][1]]
        d_t = dict(zip(keys_t, values_t))
        # j_t = json.dumps(d_t)
        d_p = dict(zip(keys_p, values_p))
        # j_p = json.dumps(d_p)
        values_u = [d_t, d_p, self[2]]
        d_u = dict(zip(keys_u, values_u))
        j_u = json.dumps(d_u)
        return (j_u)
